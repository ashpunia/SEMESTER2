...
double ImprovedDistanceFieldMethod(Image<Color> &input, Image<DistancePixel> &distance_image) 
{
  ...
  for (int x = 0; x < w; x++) 
  {
    for (int y = 0; y < h; y++) 
    {
      double closest_distance = -1; // Initialize closest distance to a negative value
      // Loop through black pixels vector to find the closest distance
      for (int m = 0; m < black_pixels.size(); m++) 
      {
        int black_pixel_x = black_pixels[m].getX();
        int black_pixel_y = black_pixels[m].getY();
        double distance = sqrt((x - black_pixel_x)*(x - black_pixel_x) + (y - black_pixel_y)*(y - black_pixel_y));
        // Update the closest distance to a black pixel if necessary
