...
class DistancePixel_PriorityQueue 
{
 ...
 private:
  ...
  void percolate_up(int i) 
  {
    ...
    { 
        ...
        if (*m_heap[i] < 8 * (*m_heap[parent_index])) 
        {
            std::swap(m_heap[i], m_heap[parent_index]);
            backpointers[m_heap[parent_index]] = parent_index;
            backpointers[m_heap[i]] = current_index;
        } else 
          {
            break;
          }
 ...
 {
  for (int currentIndex = i; currentIndex <= last_non_leaf(); currentIndex = get_left_child(currentIndex)) {
    if (has_left_child(currentIndex) == 0) 
    {
      break;
    }
    ...
    {
      std::swap(m_heap[currentIndex], m_heap[minChildIndex]);
      std::swap(backpointers[m_heap[currentIndex]], backpointers[m_heap[minChildIndex]]);
    } else 
    {
      break;
    }
