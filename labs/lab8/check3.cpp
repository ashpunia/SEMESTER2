...
public:
  ...
  bool isValid(int a, int b) { return (a==x && b==y);}
  int x,y;
};
...
void print_grid(const std::vector<std::vector<GRID_STATUS> > & blocked_grid, const std::vector<Point>& points,
...
int recurse(int x, int y, const std::vector<std::vector<GRID_STATUS> > &blocked_grid, std::vector<Point> &points){
    Point p(x, y);
    ...
    int count = 0;
    int tmp = recurse(x-1, y, blocked_grid, points);
    if (tmp == 0) tmp += recurse(x, y-1, blocked_grid, points);
    if (tmp == 0) points.pop_back();
    count = tmp;
    return count;
}
...
int main(int argc, char* argv[]) {
  ...
  print_grid(blocked_grid, points, start_x, start_y);
  int x = start_x;
  int y = start_y;
  std::cout << "number of possible paths is " << recurse(x, y, blocked_grid, points) << std::endl;
